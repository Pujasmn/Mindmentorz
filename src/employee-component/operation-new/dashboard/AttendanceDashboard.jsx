...
// import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, Coffee, PieChart, Sun, UserCheck, UserX } from 'lucide-react';
// import React, { useState } from 'react';
// import { markAttendance } from '../../../api/service/employee/EmployeeService'; // Assuming markAttendance is defined here
...
//       const formData = { employeeName }; // Send only the employee name
...
//     return currentHour >= 17 && currentHour <= 20; // 5 PM to 8 PM
//   };
...
//   const generateCalendarDays = (year, month) => {
//     const daysInMonth = getDaysInMonth(year, month); // Fixed the syntax error here
//     const firstDay = getFirstDayOfMonth(year, month);
//     const days = [];
...
//         {/* Main Calendar Section */}
//         <div className="lg:w-2/3">
//           {selectedMonth === today.getMonth() && currentYear === today.getFullYear() && (
//             <div className="mb-4 bg-white rounded-lg p-4 shadow-md border-l-4 border-[#642b8f]">
//               <div className="flex items-center justify-between">
//                 <div className="flex items-center space-x-3">
//                   <UserCheck className="w-6 h-6 text-[#642b8f]" />
...
//                   </div>
//                 </div>
//                 {!attendanceData[todayString] && (
//                   <button
//                     onClick={handleMarkAttendance}
//                     disabled={!isWithinAttendanceTime()} // Disable button outside of allowed time
//                     className={`px-4 py-2 ${isWithinAttendanceTime() ? 'bg-[#642b8f] text-white' : 'bg-gray-300 text-gray-500'} rounded-lg hover:bg-[#4e1f73] transition-colors duration-200 flex items-center space-x-2`}
//                   >
//                     <UserCheck className="w-4 h-4" />
//                     <span>Mark Present</span>
//                   </button>
...
const AttendanceCalendar = () => {
  ...
  const handleMarkAttendance = async () => {
    ...
    if (!isWithinAttendanceTime()) {
      toast.error('Attendance can only be marked between 5 PM and 8 PM', {
        position: 'top-right',
        duration: 3000,
      });
      return;
